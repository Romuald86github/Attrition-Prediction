...
def train_and_evaluate_models(X_train, y_train, X_val, y_val, X_test, y_test):
    with mlflow.start_run():
        ...
        if isinstance(best_model, RandomForestClassifier):
            ...
            mlflow.log_metric("tuned_val_accuracy", val_accuracy)
            mlflow.log_metric("tuned_test_accuracy", test_accuracy)
            mlflow.log_metric("tuned_val_precision", val_precision)
            mlflow.log_metric("tuned_test_precision", test_precision)
            mlflow.log_metric("tuned_val_recall", val_recall)
            mlflow.log_metric("tuned_test_recall", test_recall)
            mlflow.log_metric("tuned_val_f1", val_f1)
            mlflow.log_metric("tuned_test_f1", test_f1)
            mlflow.log_metric("tuned_val_roc_auc", val_roc_auc)
            mlflow.log_metric("tuned_test_roc_auc", test_roc_auc)
            mlflow.sklearn.log_model(best_model, "TunedRandomForestClassifier", registered_model_name="TunedRandomForestClassifier")
...
if __name__ == "__main__":
    ...
    best_model = train_and_evaluate_models(X_train, y_train, X_val, y_val, X_test, y_test)
